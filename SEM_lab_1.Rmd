...
4. Lear to use some packages in R
5. How:
...
It is a good practice to check that everything is in order and make sense of the data that we are going to analysise. 
...
# let's calclate the sample implied covariance matrix 
...
We can quickly check if our theoretical model has a positive degrees of freedom using the following formula.

**Degrees of freedom** = Number of parameters to estimate - Pieces of informations

**Pieces of informations** = $\dfrac{p(p+1)}{2}=\dfrac{3(3+1)}{2}=6$.
...
**Q:** How our model perform in term of global fit measures?

We can have a better grasp of what happing taking a look at the local fit of our model. 
