...
# . Mplus shows the tetrachoric correlation matrix by default. lavaan does not.   Request it using in- spect(fit,"sampstat")$cov and compare  to the Mplus output shown in Brown, p. 393
...
addmargins(table(ds$s1))
addmargins(table(ds$s2))
addmargins(table(ds$s3))
addmargins(table(ds$s4))
